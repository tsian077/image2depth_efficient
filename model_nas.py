...
def create_model_nas(existing='', is_twohundred=False, is_halffeatures=True):
    ...
    if len(existing) == 0:
        ...
        if is_twohundred:
            ...
            base_model = applications.nasnet.NASNetMobile(input_shape=(480, 640, 3), include_top=False,weights=None)

            from contextlib import redirect_stdout
            with open('model_nas_summary.txt', 'w') as f:
        ...
        decoder = upproject(decoder, int(decode_filters/8), 'up3', concat_with='separable_conv_1_reduction_left')
        decoder = upproject(decoder, int(decode_filters/16), 'up4', concat_with='stem_conv1')
