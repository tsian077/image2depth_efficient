...
from keras.layers import Input, InputLayer, Conv2D, Activation, LeakyReLU, Concatenate,Dropout,Multiply
...
def insert_layer_nonseq(model, layer_regex, insert_layer_factory,
    ...
    for layer in model.layers[1:]:
        print('layer:',layer.name) #目前的層
        
        print('last_layer:',network_dict['input_layers_of'][layer.name]) #前一個
        if re.match('block1a_project_conv',layer.name): 
            network_dict['input_layers_of'][layer.name]=['block1a_activation']
        if re.match('block1a_se_squeeze',layer.name): 
            continue
        elif re.match('block1a_se_reshape',layer.name):
            continue
        elif re.match('block1a_se_reduce',layer.name):
            continue
        elif re.match('block1a_se_expand',layer.name):
            continue
        elif re.match('block1a_se_excite',layer.name):
            continue
...
def create_model(existing='', is_twohundred=False, is_halffeatures=True):
    ...
    if len(existing) == 0:
        ...
        if is_twohundred:
            ...
            base_model = efn.EfficientNetB3(input_shape=(480,640,3),include_top=False,weights='imagenet')
        ...
        # base_model = insert_layer_nonseq(base_model,'block3a_dwconv',non_local_layer_factory)
        # base_model = insert_layer_nonseq(base_model,'no',non_local_layer_factory)
        ...
        def upproject(tensor, filters, name, concat_with):
            ...
            reduce_channel = (channels*0.25)
            se_up_i = Conv2D(filters=reduce_channel,kernel_size=1,stdires=1,padding='same',name=name+'_se_reduce')(se_up_i)
        ...
        # Decoder Layers
        decoder = Conv2D(filters=decode_filters, kernel_size=1, padding='same', input_shape=base_model_output_shape, name='conv2')(base_model.get_layer('top_activation').output)
