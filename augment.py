...
class BasicPolicy(object):
    def __init__(self, mirror_ratio = 0, flip_ratio = 0, color_change_ratio = 0, is_full_set_colors = False, add_noise_peak = 0.0, erase_ratio = -1.0):
        ...
        print('color',self.color_change_ratio)
        print('mirror',self.mirror_ratio)
        print('flip',self.flip_ratio)
    #erase_ratio=-1.0
    ...
    def __call__(self, img, depth):
        ...
        if self.add_noise_peak > 0:
            ...
            # img2 = np.random.poisson(np.clip(img2, 0, 1) * PEAK) / PEAK
        ...
        # img2 = img2[...,list(self.indices[policy_idx])]
        ...
        if random.uniform(0, 1) < self.erase_ratio:
            img = self.eraser(img)
            # img2 = self.eraser(img2)
        # # 5) solarize
        # # https://github.com/open-mmlab/mmcv/pull/132/files
        # if random.uniform(0,1) < 0.5:
        #     # print(type(img))
        #     img = np.where(img < (128/255), img, (255/255) - img)
    ...
    def debug_img(self, img, depth, idx, i, prefix=''):
        ...
        aug_img.save('/home/jimmy/DenseDepth/debug_augementdata/'+str(idx)+"_"+str(i)+'.jpg',quality=99)
        aug_img = Image.fromarray(np.clip(np.uint8(np.tile(depth*255,3)), 0, 255))
        aug_img.save('/home/jimmy/DenseDepth/debug_augementdata/'+str(idx)+"_"+str(i)+'.depth.jpg',quality=99)
