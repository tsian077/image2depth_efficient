...
def get_nyu_data(batch_size, nyu_data_zipfile='nyu_data_unreal.zip'):
    data = extract_zip(nyu_data_zipfile)
    
    nyu2_train = list((row.split(',') for row in (data['data/nyu2_train3.csv']).decode("utf-8").replace("\r","").split('\n') if len(row) > 0))
...
class NYU_BasicAugmentRGBSequence(Sequence):
    ...
    def __getitem__(self, idx, is_apply_policy=True):
        batch_x, batch_y,batch_rex = np.zeros( self.shape_rgb ), np.zeros( self.shape_depth ),np.zeros((self.batch_size, 240, 320, 3))
        ...
        for i in range(batch_x.shape[0]):
            ...
            #image為(480,640,3) depth為(480,640) 0~255的值
            #image/255.0並限制在0~1  depth會/255.0再乘以1000.0的最大深度值
            # print(BytesIO(self.data[sample[0]]))
            ...
            if is_apply_policy: 
                ...
                # batch_x[i], batch_y[i],batch_rex[i] = self.policy(batch_x[i], batch_y[i],batch_rex[i])
                # print('policy')
                # print('batch_x[i]',batch_x[i])
                # print('batch_y[i]',batch_y[i])

            # DEBUG:
            # self.policy.debug_img(batch_x[i], np.clip(DepthNorm(batch_y[i],1000)/1000,0,1), idx, i)
        #exit()
        patch = int(240 / 2**4)
        patch2 = int(360 / 2**4)
        disc_patch = (patch, 20, 1)
        fake = np.zeros((self.batch_size,)+ disc_patch)
        valid = np.ones((self.batch_size,)+ disc_patch)
        #return [batch_x,batch_y,batch_rex], [fake,valid,batch_y]
...
class Unreal_BasicAugmentRGBSequence(Sequence):
    def __init__(self, data, dataset, batch_size, shape_rgb, shape_depth, is_flip=False, is_addnoise=False, is_erase=False, is_skip_policy=False):
        self.data = data
        self.dataset = dataset
        self.policy = BasicPolicy( color_change_ratio=0.50, mirror_ratio=0.90, flip_ratio=0.0 if not is_flip else 0.2, 
                                    add_noise_peak=0 if not is_addnoise else 20, erase_ratio=-1.0 if not is_erase else 0.5)
        self.batch_size = batch_size
        self.shape_rgb = shape_rgb
        self.shape_depth = shape_depth
