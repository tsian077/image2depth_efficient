...
from utils import DepthNorm, predict, evaluate,nyu_resize
        ...
        def on_epoch_end(self, epoch, logs=None):            
            if not test_set == None:
                ...
                for i in range(self.num_samples):
                    ...
                    #resize x
                    x_train_resize = nyu_resize(x_train,240)
                    x_test_resize  = nyu_resize(x_test,240)
                    ...
                    # predict_train = plasma(predict(model, x_train,y_train,x_train_resize, minDepth=minDepth, maxDepth=maxDepth)[0,:,:,0])[:,:,:3]
                    # predict_test = plasma(predict(model, x_test,y_test,x_test_resize, minDepth=minDepth, maxDepth=maxDepth)[0,:,:,0])[:,:,:3]
                ...
                fp = open(runPath+"/trian_eval.txt","a+")
