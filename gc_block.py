...
def gc_block(ip, intermediate_dim=None, compression=2,
    ...
    if mode not in ['gaussian', 'embedded', 'dot', 'concatenate','gc']:
    ...
    if mode == 'gaussian':  # Gaussian instantiation
        ...
        print('ip',ip)
        #input [N,H,W,C]                                           [None,60,80,64]
        context_mask = _convND(ip,rank,1)  # [N,H,W,1]             [None,60,80,1]
        # print('1',context_mask)
        context_mask = Reshape((-1,1))(context_mask) # [N,H*W,1]   [None,4800,1]
        # print('2',context_mask)
        context_mask = Permute((2,1))(context_mask)  #[N,1,H*W]      [None,1,4800]
        # print('3',context_mask)
        cont ext_mask = Activation('softmax')(context_mask)    #    [None,1,4800]
        # print('4',context_mask)
        input_x = Reshape((-1,channels))(ip)     #input[None,H*W,C]    [None,4800,C]
        # print('C',channels)
        # print('input',input_x)
        # context = K.batch_dot(context_mask,input_x)      # [None,1,C]
        # context = dot([context_mask,input_x],axes=(2,1))
        # print('context',context)
        def matmul(IP):
            context_mask=IP[0]
            input_x=IP[1]
            return K.batch_dot(context_mask,input_x)


        # context = Lambda(lambda context_mask,input_x: K.batch_dot(context_mask,input_x))([context_mask,input_x])
        context = Lambda(matmul)([context_mask,input_x])
        
        # print('context',context)
        y =  Lambda(lambda context:K.expand_dims(context,axis=1))(context)
        # y = K.expand_dims(context,axis=1)          #[None,1,1,C] 
        print(y)        
