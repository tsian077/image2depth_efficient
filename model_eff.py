...
def create_model_eff(existing='', is_twohundred=False, is_halffeatures=True):
    ...
    if len(existing) == 0:
        ...
        if is_twohundred:
            ...
            base_model = efn.EfficientNetB5(input_shape=(480,640,3),include_top=False,weights='noisy-student') 
            # base_model = efn.EfficientNetB6(input_shape=(480,640,3),include_top=False,weights='imagenet')
            # base_model = efn.EfficientNetB6(input_shape=(480,640,3),include_top=False,weights='noisy-student')
        ...
        def upproject(tensor, filters, name, concat_with):
            ...
            up_i = PixelShuffler((2,2),name=name+'_Pixelshuffle2d')(tensor)
            ...
            up_i = Conv2D(filters=filters*2, kernel_size=3, strides=1, padding='same', name=name+'_convA')(up_i)
            up_i =  GroupNormalization(groups=16, axis=-1)(up_i)# bs5


            # up_i =  GroupNormalization(groups=18, axis=-1)(up_i)
            # up_i = BatchNormalization(axis=3)(up_i)
        ...
        decoder = Concatenate(name='conv2_concat')([decoder, base_model.get_layer("block6i_add").output]) # Skip connection
        decoder = Conv2D(filters=int(decode_filters/2), kernel_size=1, strides=1, padding='same', name='conv2_convA')(decoder)
        decoder = GroupNormalization(groups=16, axis=-1,name='conv2_GNL')(decoder)# bs5
        decoder = LeakyReLU(alpha=0.2,name='con2_LRel')(decoder)
        print(decoder)
        ...
        decoder = upproject(decoder, int(decode_filters/2), 'up1', concat_with='block4g_add')  #block4a_dwconv
        decoder = upproject(decoder, int(decode_filters/4), 'up2', concat_with='block3e_add')  #block3a_dwconv
        decoder = upproject(decoder, int(decode_filters/8), 'up3', concat_with='block2e_add')  #block2a_dwconv
        ...
        custom_objects = {'BilinearUpSampling2D': BilinearUpSampling2D,'depth_loss_function': depth_loss_function,'LayerNormalization':LayerNormalization,'PixelShuffler':PixelShuffler,'GroupNormalization':GroupNormalization}
